...
namespace Common.Gen
{
    public class HelperSysObjectsVue : HelperSysObjectsBaseFront
    {
        ...
        private void ExecuteTemplateVueRouter(TableInfo tableInfo, Context configContext)
        {
            ...
            {
                var str1 = @"    { path: '" + item.ClassName.ToLower() + "', name: '" + (item.ClassNameFormated ?? item.ClassName) + "', component: function (resolve) { require(['@/views/" + item.ClassName.ToLower() + "'], resolve) } },";
                var str2 = @"    { path: '" + item.ClassName.ToLower() + "/create', name: 'Novo " + (item.ClassNameFormated ?? item.ClassName) + "', component: function (resolve) { require(['@/views/" + item.ClassName.ToLower() + "/create'], resolve) } },";
                var str3 = @"    { path: '" + item.ClassName.ToLower() + "/edit/:id', name: 'Editar " + (item.ClassNameFormated ?? item.ClassName) + "', component: function (resolve) { require(['@/views/" + item.ClassName.ToLower() + "/edit'], resolve) } },";
