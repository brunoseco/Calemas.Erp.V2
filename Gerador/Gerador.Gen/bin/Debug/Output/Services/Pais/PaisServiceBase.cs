...
{
    public class PaisServiceBase : ServiceBase<Pais, PaisDto, PaisFilter>, IService
    {
        protected PaisRepository _rep;
        protected CurrentUser _user;

        public PaisServiceBase(PaisRepository rep, ValidationContract validation, CurrentUser user, IUnitOfWork uow)
		...
		public virtual async Task<IEnumerable<dynamic>> GetDataItems(PaisFilter filters)
        {
        ...
        public virtual async Task<SearchResult<PaisDto>> GetByFiltersPaging(PaisFilter filters)
        {
        ...
        public virtual async Task<PaisDto> GetById(PaisFilter filters)
        {
            var alvo = this._rep.GetById(filters);
            var result = await MapperDomainToDto<PaisDtoDetail>(alvo);
            return result;
        }
		
        public virtual Pais GetByModel(Pais model)
        {
            var alvo = this._rep.GetByModel(model);
            return alvo;
        }

        public override Pais Save(Pais model)
        {
        ...
        public virtual void MakeValidations(Pais model, Pais modelOld)
        {
            if (model.IsNull())
